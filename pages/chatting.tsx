    ...
    <>
      <Section>
        <div className='flex items-center justify-center h-full'>
          <h1 className='text-7xl'>페이지 타이틀</h1>
        </div>
      </Section>
      {data.map((v, i) => (
        <Section key={i}>
          <div className='box-border relative mx-20' ref={v.ref}>
            <h2 className='z-10 text-xl font-bold text-discordGray-600'>
              {v.title}
            </h2>
            <p className='z-10 w-2/5'>{v.text}</p>
            <div
              className={`fixed duration-300 left-1/4 top-1/4 ${
                v.active ? 'opacity-1' : 'opacity-0'
              }`}
            >
              <Image
                src={v.image}
